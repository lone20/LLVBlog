    ...
    public class LoginController : Controller
    {
        ...
        private void SendResetPass(Account user)
        {
            ...
            using (MailMessage mm = new MailMessage("hvlshopresetpass@gmail.com", user.Gmail))
            {
                ...
                body += "<br /><a href = '" + string.Format("{0}://{1}/Login/ResetPass/{2}", Request.Url.Scheme, Request.Url.Authority, resetCode) + "'>Chọn tại đây để tạo lại mật khẩu của bạn </a>";
                body += "<br /><br />Cảm ơn";
                mm.Body = body;
                mm.IsBodyHtml = true;
                SmtpClient smtp = new SmtpClient();
        ...
        public ActionResult ResetPass()
        {
            if (RouteData.Values["id"] != null)
            {
                ...
                AccountResetPass accountReset = _db.AccountResetPasses.Where(p => p.ResetPassCode == ResetCode).FirstOrDefault();
                ResetPasswordModel model = new ResetPasswordModel();
                if (accountReset != null)
                {
                    model.ResetCode = accountReset.ResetPassCode;
                    return View(model);
                }
        ...
        public ActionResult ResetPass(ResetPasswordModel model)
        {
            ...
            if (model != null)
            {
                var accountResetPass = _db.AccountResetPasses.Where(p => p.ResetPassCode == model.ResetCode).FirstOrDefault();
